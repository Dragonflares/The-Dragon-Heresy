...
export class CalcCommand extends GenericCommand {
    constructor(plugin) {
        super(plugin, {
            name: 'calculate',
            aliases: ['calc'],
    ...
    async run(message, args) {
        let calculate = "=" + message.content.toLowerCase().substring(5)
        if (isFinite(calculate.replace(/.|\=|\+|\-|\*|\/|\÷|\%|\(|\)|\,|\ |math.|pow|sqrt|round|floor|ceiling|ceil|pi|π|euler|absolute|abs|exp|logarithm|log|random|rand|rng/g,''))) {
          ...
          if (isFinite(eval(calculate))) message.channel.send(eval(calculate).toFixed(2));
        }
    }
}