...
export class RecruitWhiteStarCommand extends WhitestarsCommand {
  ...
  roleMessage = async (message, role, member) => {
    ...
    const recruitButtons = await WsMessages.whiteStarRecruitButtons(newWhiteStar)

    //Send Message
    const messageReaction = await message.channel.send({ embeds: [recruitEmbed], components: recruitButtons});
