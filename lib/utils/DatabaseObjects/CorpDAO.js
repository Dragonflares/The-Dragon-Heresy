...
class DAOCorp {
    ...
    */
    static async find(CorpsId) {
        let corp = await Corp.findOne({corpId: CorpsId.toString()})
        if(!corp) {
            return null
        }
        else return corp
     ...
     * @return Corporation
    */
    static async removeFromCorp(member, Corp) {
        if(!member.Corp){}
        else if (member.Corp.toString() === Corp._id.toString()){
            let populatedMember = await Member.findOne({_id: member._id}).populate('Corp').exec()
            let oldCorp = await this.find(populatedMember.Corp.corpId)
            if(!oldCorp) {}
            else {
                let remainingMembers = oldCorp.members.filter(aMember => aMember.toString() != populatedMember._id.toString())
