...
module.exports = {
    ...
    usage: "&setplayerbattleship, then asnwer the bot's questions. Don't state any levels unless asked for. You can also modify weapon, supports, and shield indiviually.",
    run: async (client, message, args) => {
        ...
        if(!messagesplitted[1]) {
            ...
            client.playerDB.set(`${message.author.id}`, `${battleshipname.first().content}`, `battleship.name`)

            client.playerDB.set(`${message.author.id}`, battleshiplevel.first().content , `battleship.level`)
            ...
            client.playerDB.set(`${message.author.id}`, [], `battleship.support`)
            ...
            client.playerDB.set(`${message.author.id}`, `${battleshipshield.first().content} ${techlevelshield}`, `battleship.shield`)
                ...
                return message.channel.send("Either invalid paramter to change, or you tried ot change the name or the level of the battleship. For that, the entire ship must be reset.")
            }
            return message.channel.send("Your battleship for white stars is now set.")
        }
    }
