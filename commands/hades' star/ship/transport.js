...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        let transportname = client.playerDB.get(`${targetid}`, `transport.name`)
        ...
        let transportlevel = client.playerDB.get(`${targetid}`, `transport.level`)
        transportEmbed.addField("Level", `${transportlevel}`)
        let transportsupport = client.playerDB.get(`${targetid}`, `transport.support`)
        transportEmbed.addField("Support", `${transportsupport}`)
        if(transportlevel > 1) {
            let transporteconomys = client.playerDB.get(`${targetid}`, `transport.economy`)
            let economys = ""
